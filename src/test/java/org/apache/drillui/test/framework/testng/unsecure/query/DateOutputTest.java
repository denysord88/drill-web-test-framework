...
import org.apache.drillui.test.framework.testng.EventsListener;
...
public class DateOutputTest extends BaseUnsecureTest {
  ...
  public void selectDateQuery() {
    QuerySteps.runQuery("slect date '2017-04-06';");
    String error = EventsListener.getError();
  ...
  public void selectTimestampTwoQuery() {
    ...
    QuerySteps.runQuery("SELECT * as testAlias FROM cp.`employee.json` LIMIT 2");
    String error = EventsListener.getError();
    String result = (String) QueryResultsSteps.getRow(0).get(0);
